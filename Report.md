...
There is already a working implementation of PPO algorithm in the repository under the **agents.py** module. Work is in progress to tune the PPO agent hyperparameters such that the agent can solve the environment sooner than DDPG. If you have suggestions on the PPO implementation, feel free to raise an issue.
